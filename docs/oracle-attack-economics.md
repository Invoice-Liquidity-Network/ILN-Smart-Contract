...
| Legacy 7-day staleness constant | `ORACLE_STALENESS_THRESHOLD_SECS` = 604 800s | `oracle_interface.rs:46` | **Dead code** — `check_payer_verified()`, the only function that reads it, is never called from `fund_invoice` or anywhere else in the live path. `oracle-integration.md`'s "Staleness Policy" section describes this constant as the enforced policy; it is not — see [§6](#6-a-stale-doc-this-model-corrects). |
