...
fn test_pending_to_partially_funded_to_funded() {
    ...
    t.contract.fund_invoice(&funder1, &id, &t.token.address, &partial_amount);
    ...
    t.contract.fund_invoice(&funder2, &id, &t.token.address, &remaining_amount);
...
fn test_all_reachable_states_from_pending() {
    ...
    t1.contract.fund_invoice(&t1.funder, &id1, &t1.token.address, &INVOICE_AMOUNT);
    ...
    t2.contract.fund_invoice(&t2.funder, &id2, &t2.token.address, &INVOICE_AMOUNT);
    ...
    t3.contract
        .fund_invoice(&funder1, &id3, &t3.token.address, &t3.token.address, &(INVOICE_AMOUNT * 3000 / 10000));
    assert_eq!(
    ...
    t3.contract
        .fund_invoice(&funder2, &id3, &t3.token.address, &t3.token.address, &(INVOICE_AMOUNT * 7000 / 10000));
    assert_eq!(
        t3.contract.get_invoice(&id3).status,
        InvoiceStatus::Funded,
...
fn test_state_transition_matrix_validation() {
    ...
    t.contract
        .fund_invoice(&t.funder, &id_funded, &t.token.address, &INVOICE_AMOUNT);
    ...
    t.contract
        .fund_invoice(&t.funder, &id_default, &t.token.address, &INVOICE_AMOUNT);
...
fn test_cannot_mark_paid_on_partially_funded_invoice() {
    ...
    // Fund only 50%
    t.contract.fund_invoice(&funder, &id, &t.token.address, &(INVOICE_AMOUNT / 2));
...
fn test_overfunding_rejected_preserves_state() {
    ...
    t.contract
        .fund_invoice(&funder1, &id, &t.token.address, &(INVOICE_AMOUNT * 6000 / 10000));
    ...
    let result = t.contract.try_fund_invoice(&funder2, &id, &t.token.address, &overfund_amount);
...
fn test_exact_multi_party_funding_completion() {
    ...
    t.contract
        .fund_invoice(&funder1, &id, &t.token.address, &(INVOICE_AMOUNT * 2500 / 10000));
    assert_eq!(
        t.contract.get_invoice(&id).status,
        InvoiceStatus::PartiallyFunded
    );
    ...
    t.contract
        .fund_invoice(&funder2, &id, &t.token.address, &(INVOICE_AMOUNT * 3500 / 10000));
    assert_eq!(
        t.contract.get_invoice(&id).status,
        InvoiceStatus::PartiallyFunded
    );
    ...
    t.contract
        .fund_invoice(&funder3, &id, &t.token.address, &(INVOICE_AMOUNT * 4000 / 10000));
    assert_eq!(
        t.contract.get_invoice(&id).status,
        InvoiceStatus::Funded,
